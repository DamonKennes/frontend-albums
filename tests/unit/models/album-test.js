...
module('Unit | Model | album', function (hooks) {
  ...
  test('it has correct attributes', function (assert) {
    ...
    assert.strictEqual(model.cover, 'https://example.com/cover.jpg', 'cover attribute exists');
    assert.strictEqual(model.genre, 'Rock', 'genre attribute exists');
    assert.strictEqual(model.averagerating, 4.5, 'averagerating attribute exists');
  });
  ...
  test('it has artists relationship', async function (assert) {
    ...
    assert.strictEqual(albumArtists[0].name, 'The Beatles', 'artist relationship works');
  });
